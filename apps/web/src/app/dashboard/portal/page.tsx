...
import {
  ...
  Clock, CheckCircle2, Circle, Loader2, X, FileText,
...
export default function SelfServicePortal() {
  ...
  const sendMessage = async (overrideText?: string) => {
    ...
    try {
      let sid = sessionId;
      if (!sid) {
        const res = await api.post<{ data: { id: string } }>('/ai/sessions', { title: text.substring(0, 40) });
        sid = res.data.id;
        setSessionId(sid);
      }
      const res = await api.post<{ data: { content: string } }>('/ai/chat', { session_id: sid, message: text.trim() });
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ position:'relative', maxWidth:640 }}>
            ...
            />
            {search && (
              <button onClick={() => { setSearch(''); setKbSearch(''); }} style={{ position:'absolute', right:12, top:'50%', transform:'translateY(-50%)', background:'none', border:'none', cursor:'pointer', color:'#94a3b8', display:'flex' }}>
                <X size={16} />
              </button>
            )}
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 320px', gap:20, marginBottom:24 }}>
          ...
          <div className="ssp-card" style={{
            ...
            <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border-subtle)', background:'var(--bg-secondary)', display:'flex', alignItems:'center', gap:10, flexShrink:0 }}>
              ...
              {chatStarted && (
                <button onClick={() => { setMessages([]); setSessionId(null); setChatStarted(false); }} style={{ marginLeft:'auto', background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', fontSize:11, display:'flex', alignItems:'center', gap:4 }}>
                ...
                <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', height:'100%', textAlign:'center', padding:'0 20px' }}>
                  ...
                  <div style={{ display:'flex', flexWrap:'wrap', gap:6, justifyContent:'center' }}>
                    {['My computer is slow', 'I need VPN access', 'Reset my password', 'Track my ticket'].map(s => (
                      <button key={s} onClick={() => sendMessage(s)} style={{ padding:'6px 12px', background:'var(--bg-secondary)', border:'1px solid var(--border)', borderRadius:20, fontSize:12, fontWeight:500, color:'var(--text)', cursor:'pointer' }}>
                        {s}
                      </button>
                    ))}
                  </div>
            ...
            {/* Input */}
            <div style={{ padding:'12px 14px', borderTop:'1px solid var(--border-subtle)', background:'var(--bg-secondary)', flexShrink:0 }}>
              <div style={{ display:'flex', gap:8, alignItems:'flex-end', background:'var(--card)', border:'1.5px solid var(--border)', borderRadius:10, padding:'6px 6px 6px 12px', transition:'border-color 0.2s' }}
              ...
              {!showTicketForm ? (
                <div style={{ padding:16 }}>
                  ...
                  <button
                    ...
                    style={{ width:'100%', padding:'10px', background:'linear-gradient(135deg,#2563eb,#4f46e5)', color:'white', border:'none', borderRadius:8, fontSize:13, fontWeight:600, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', gap:6 }}
                  >
          ...
          <div className="ssp-card" style={{ background:'var(--card)', border:'1px solid var(--border)', borderRadius:16, overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.06)' }}>
            <div style={{ padding:'14px 18px', borderBottom:'1px solid var(--border-subtle)', background:'var(--bg-secondary)', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
              ...
              <button
                ...
                style={{ display:'flex', alignItems:'center', gap:6, padding:'6px 12px', background:'#7c3aed', color:'white', border:'none', borderRadius:6, fontSize:12, fontWeight:600, cursor:'pointer' }}
              >
                ...
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {sspDocs.map(doc => (
                    <div key={doc.id} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 12px', background:'var(--bg-secondary)', borderRadius:8, border:'1px solid var(--border-subtle)' }}>
                      ...
                      <button onClick={() => setSspDocs(prev => prev.filter(d => d.id !== doc.id))} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-muted)', display:'flex' }}><Trash2 size={13} /></button>
                    </div>
                  ))}
                </div>
              )}
