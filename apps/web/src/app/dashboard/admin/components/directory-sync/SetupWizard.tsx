...
import { CheckCircle, ExternalLink, Link2, ChevronDown, Cloud } from 'lucide-react';
...
export function SetupWizard({
  ...
  provider?: 'google_workspace' | 'azure_ad';
}) {
            ...
            return (
              <div key={step.key} style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ paddingBottom: isLast ? 0 : 16, flex: 1, minWidth: 0 }}>
                  ...
                  {isCurrent && step.key === 'credentials' && provider === 'google_workspace' && (
                  ...
                  {isCurrent && step.key === 'credentials' && provider === 'azure_ad' && (
                    <div style={{ marginTop: 10, display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                      <a
                      ...
                      >
                        <ExternalLink size={11} />
                        <span className="hide-mobile">Open Azure Portal</span>
                        <span className="show-mobile">Azure Portal</span>
                      </a>
                      <span className="hide-mobile" style={{ fontSize: '11px', color: 'var(--text-muted)' }}>
                        Register a new application in Azure AD App Registrations
                      </span>
                    </div>
                  )}

                  {isCurrent && step.key === 'oauth' && provider === 'google_workspace' && (
                    <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {hasUnsavedChanges && (
                        <div style={{
                          ...
                          Save your credentials first using the "Save Changes" button below before connecting.
                        </div>
                      )}
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                        <span
                          ...
                          role="button"
                          tabIndex={0}
                          onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); onConnectOAuth?.(); } }}
                          style={{
