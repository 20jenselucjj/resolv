...
export function AgentSettingsTab({ showAlert }: {
  ...
  useEffect(() => {
    Promise.all([
      ...
      api.get<{ data: AgentStats }>('/assets/stats').then(res => {
        if (res.data) {
          const d = res.data;
          const byStatus = (d as any).agentStatus || [];
          setAgentStats({
            total: d.total || 0,
            online: byStatus.find((s: any) => s.agent_status === 'online')?.count || 0,
            offline: byStatus.find((s: any) => s.agent_status === 'offline')?.count || 0,
            never_connected: byStatus.find((s: any) => s.agent_status === 'never')?.count || 0,
          });
        }
          ...
          <div style={{ flex: 1, minWidth: 200 }}>
            <div style={{
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 16 }}>
                Single .exe &bull; ~42 MB &bull; Windows Service via nssm
              </div>
              <button
