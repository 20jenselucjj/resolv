          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px', flexWrap: 'wrap' }}>
            ...
            <div style={{ width: DONUT_SIZE, height: DONUT_SIZE, position: 'relative', flexShrink: 0 }}>
              <svg width={DONUT_SIZE} height={DONUT_SIZE} viewBox={`0 0 ${DONUT_SIZE} ${DONUT_SIZE}`}>
                ...
                {/* Segments */}
                {sortedSourceTypes.reduce<Array<{item: typeof sortedSourceTypes[0]; startAngle: number; endAngle: number; angle: number}>>((acc, item) => {
                  const angle = totalSourceHits > 0 ? (item.hits / totalSourceHits) * 360 : 0;
                  const prevAngle = acc.length > 0 ? acc[acc.length - 1].endAngle : 0;
                  return [...acc, { item, startAngle: prevAngle, endAngle: prevAngle + angle, angle }];
                }, []).filter(seg => seg.angle > 0).map((seg) => {
                  const { startAngle, endAngle, angle, item } = seg;
