...
export function AITrainingTab({ showAlert }: { showAlert: (m: string, t?: 'success' | 'error') => void }) {
  ...
  // Real-time updates: refresh sources when AI knowledge changes
  useEffect(() => {
    ...
    socket.on('knowledge:source:created', handleSourceChange);
    socket.on('knowledge:source:updated', handleSourceChange);
    socket.on('knowledge:source:deleted', handleSourceChange);

    return () => {
      socket.off('knowledge:source:created', handleSourceChange);
      socket.off('knowledge:source:updated', handleSourceChange);
      socket.off('knowledge:source:deleted', handleSourceChange);
    };
  }, [activeSubTab]);

  // --- Initial Data Load ---
  const loadTabSpecificData = useCallback(async () => {
    setLoading(true);
    try {
      if (activeSubTab === 'sources') {
        const res = await api.get<{ data: KnowledgeSource[] }>('/ai/knowledge/sources');
  ...
  }, [activeSubTab, showAlert]);

  useEffect(() => {
    const timer = setTimeout(() => loadTabSpecificData(), 0);
  ...
  const handleAddSource = async (e: React.FormEvent) => {
    ...
    if (sourceForm.source_type === 'file') {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to upload file', 'error');
        setUploading(false);
        return;
      }
      setUploading(false);
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to save knowledge source', 'error');
    }
  };
  ...
  const toggleSourceActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to toggle source status', 'error');
    }
  };
  ...
  const handleReprocessSource = async (id: string) => {
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to dispatch reprocess request', 'error');
    }
  };
  ...
  const handleDeleteSource = async (id: string) => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to delete knowledge source', 'error');
    }
  };
  ...
  const handleBulkDeleteSources = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to delete sources', 'error');
    }
  };
  ...
  const handleBulkReprocessSources = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to reprocess sources', 'error');
    }
  };
  ...
  const handleBulkToggleSources = async (activate: boolean) => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to update sources', 'error');
    }
  };
  ...
  const handleSyncKB = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to initiate KB sync', 'error');
    } finally {
      setSyncingKB(false);
    }
  };
  ...
  const handleSyncTickets = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to initiate ticket sync', 'error');
    } finally {
  ...
  const handleAddQA = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to create Q&A pair', 'error');
    }
  };
  ...
  const handleSaveEditQA = async (id: string) => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to update Q&A pair', 'error');
    }
  };
  ...
  const toggleQAActive = async (id: string, currentStatus: boolean) => {
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to toggle Q&A pair status', 'error');
    }
  };
  ...
  const handleDeleteQA = async (id: string) => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to delete Q&A pair', 'error');
    }
  };
  ...
  const handleSaveRAGConfig = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to save retrieval configuration', 'error');
    } finally {
  ...
  const handleRunTest = async () => {
    ...
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to test retrieval strategy', 'error');
    } finally {
      setTesting(false);
    }
  };
  ...
  const handleFlagQuery = async (id: string) => {
    try {
      ...
      showAlert(err?.serverError || err?.message || 'Failed to flag query', 'error');
    }
  };
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{
        ...
        {[
          ...
          <button
            key={key}
            onClick={() => setActiveSubTab(key as 'sources' | 'qa' | 'rag' | 'test' | 'analytics')}
            style={{
