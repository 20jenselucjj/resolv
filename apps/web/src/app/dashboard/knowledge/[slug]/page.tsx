...
export default function ArticleDetailPage() {
  ...
  // Real-time updates: live-update article when edited by someone else
  useEffect(() => {
    const socket = connectSocket();

    const handleUpdated = ({ article: updated }: { article: Article }) => {
      if (article && updated.id === article.id) {
        setArticle(updated);
      }
    };
    ...
    const handleDeleted = ({ id }: { id: string }) => {
      if (article && id === article.id) {
        setConfirmModal({
          ...
          message: 'This article has been archived by another admin.',
          onConfirm: () => router.push('/dashboard/knowledge'),
        });
      }
    };

    socket.on('knowledge:updated', handleUpdated);
    socket.on('knowledge:deleted', handleDeleted);

    return () => {
      socket.off('knowledge:updated', handleUpdated);
      socket.off('knowledge:deleted', handleDeleted);
    };
  }, [article, router]);
