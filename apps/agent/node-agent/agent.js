...
// Collects system inventory, enables remote desktop.
// CLI modes:
...
function readEmbeddedConfig() {
  try {
    const data = fs.readFileSync(process.execPath, 'utf8');
    const marker = '---RESOLV_CONFIG_START---\n';
    const endMarker = '\n---RESOLV_CONFIG_END---';
    const start = data.lastIndexOf(marker);
    if (start === -1) return null;
    const end = data.lastIndexOf(endMarker);
    if (end === -1) return null;
    return JSON.parse(data.substring(start + marker.length, end));
  } catch {
    return null;
  }
}
...
// ---------------------------------------------------------------------------
// Screenshot capture via screenshot-desktop (avoids PowerShell AV blocks).
function captureScreenBase64(quality = 60) {
  void quality;
  return screenshot({ format: 'jpg' }).then((img) => {
...
async function captureAndSendFrame(sessionId, quality = 60) {
  if (!socket || !socket.connected) return;
  if (captureInFlight[sessionId]) return;
  captureInFlight[sessionId] = true;
  try {
    const b64 = await captureScreenBase64(quality);
    if (!b64) {
      console.log('[Resolv Agent] Screenshot empty for session:', sessionId);
      return;
    }
    if (socket && socket.connected) {
      socket.emit('remote:frame', { sessionId, frame: b64 });
      // Log first frame of each session for debugging
      if (!capturedSessions[sessionId]) {
        capturedSessions[sessionId] = true;
        console.log('[Resolv Agent] Frame stream started for session:', sessionId, 'len:', b64.length);
      }
    }
  } catch (err) { console.error('[Resolv Agent] Screenshot error:', err.message); }
  finally { captureInFlight[sessionId] = false; }
}
...
function runPowerShell(script) {
  const proc = spawn('powershell', ['-NoProfile', '-Command', script], { windowsHide: true, stdio: 'ignore' });
  proc.on('error', (err) => console.error('[Resolv Agent] PowerShell error:', err.message));
}
...
function handleInput(type, payload) {
  try {
    ...
    if (type === 'mousemove') {
      ...
      runPowerShell('Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.Cursor]::Position = New-Object System.Drawing.Point(' + payload.x + ', ' + payload.y + ')');
    } else if (type === 'click') {
      runPowerShell('Add-Type -TypeDefinition @"\nusing System;\nusing System.Runtime.InteropServices;\npublic class MouseClicker {\n  [DllImport("user32.dll")]\n  public static extern void mouse_event(uint dwFlags, uint dx, uint dy, uint dwData, System.IntPtr dwExtraInfo);\n}\n"@;\n[MouseClicker]::mouse_event(0x02, 0, 0, 0, [System.IntPtr]::Zero);\n[MouseClicker]::mouse_event(0x04, 0, 0, 0, [System.IntPtr]::Zero);');
    } else if (type === 'keydown') {
      if (typeof payload.key !== 'string' || payload.key.length > 32) return;
      const safeKey = payload.key.replace(/'/g, "''");
      runPowerShell("Add-Type -AssemblyName System.Windows.Forms; [System.Windows.Forms.SendKeys]::SendWait('" + safeKey + "')");
    } else if (type === 'keyup') {
      return;
    } else if (type === 'wheel') {
      return;
    }
  } catch (err) { console.error('[Resolv Agent] Input handler error:', err.message); }
}
...
function connectSocket() {
  ...
  socket.on('remote:session:start', function (_a) {
    var sessionId = _a.sessionId, quality = _a.quality, interval = _a.interval;
    console.log('[Resolv Agent] Remote session started:', sessionId, 'quality:', quality, 'interval:', interval, 'ms');
    delete capturedSessions[sessionId];
    var capInterval = interval || 200;
    var capQuality = quality != null ? quality : 60;
  ...
  socket.on('remote:frame:request', function (_a) {
    ...
    return captureAndSendFrame(sessionId, quality);
  });
  socket.on('remote:input', function (_a) {
    var type = _a.type, payload = _a.payload;
    return handleInput(type, payload);
  });
  socket.on('remote:leave', function (payload) {
    var sessionId = typeof payload === 'string' ? payload : payload && payload.sessionId;
    if (!sessionId) return;
    console.log('[Resolv Agent] Remote session ended:', sessionId);
