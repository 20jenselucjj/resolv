import { app, BrowserWindow, Tray, Menu, nativeImage, ipcMain, Notification, desktopCapturer, screen, session } from 'electron';
...
import { RemoteDesktopHandler } from './remote';
import { H264StreamHandler } from './h264-stream';
...
let remoteHandler = new RemoteDesktopHandler();
let h264Stream = new H264StreamHandler();
...
let currentCaptureAssetId: string | null = null;
let captureWindowReady = false;
let queuedCaptureStart: { sessionId: string; assetId: string } | null = null;
...
// ─── WebRTC Capture IPC (registered once) ─────────────────────────────────────

function setupCaptureIPC(): void {
  ipcMain.on('capture:webrtc-signal', (_event, data: any) => {
    if (!socket) return;
    if (data.type === 'sdp') {
    ...
    } else if (data.type === 'capture-ready') {
      captureWindowReady = true;
      console.log('[Capture] capture window ready');
      if (queuedCaptureStart) {
        captureWindow?.webContents.send('capture:start', queuedCaptureStart);
        queuedCaptureStart = null;
      }
    } else if (data.type === 'capture-error') {
      console.error('[Capture] capture error:', data.error);
      socket.emit('webrtc:error', { sessionId: data.sessionId, error: data.error });
    } else if (data.type === 'capture-ended') {
      console.log('[Capture] capture ended by OS');
      socket.emit('webrtc:ended', { sessionId: data.sessionId });
    }
  });
}

// ─── WebRTC Capture Window ───────────────────────────────────────────────────
...
function startCaptureSession(sessionId: string): void {
  currentCaptureSessionId = sessionId;
  const config = getConfig();
  currentCaptureAssetId = config.assetId || null;

  ensureCaptureWindow();
  if (!captureWindow) {
    console.error('[Capture] Failed to create capture window, falling back to JPEG');
    return;
  }

  if (captureWindowReady) {
    captureWindow.webContents.send('capture:start', { sessionId, assetId: config.assetId });
  } else {
    queuedCaptureStart = { sessionId, assetId: config.assetId || '' };
  }
  console.log('[Capture] Session started:', sessionId);
}

function stopCaptureSession(): void {
  if (captureWindow && !captureWindow.isDestroyed()) {
    captureWindow.webContents.send('capture:stop');
  }
  currentCaptureSessionId = null;
  currentCaptureAssetId = null;
  queuedCaptureStart = null;
...
function connectSocket(config: Config): void {
  ...
  // Remote desktop: start WebRTC streaming when server requests
  socket.on('remote:session:start', (data: { sessionId: string; quality?: number; interval?: number }) => {
    console.log('Remote session start requested:', data.sessionId);
    // Transition to WebRTC-based capture
    startCaptureSession(data.sessionId);
  });

  // Remote desktop: stop streaming
  socket.on('remote:session:stop', () => {
    console.log('Remote session stop requested');
    stopCaptureSession();
    remoteHandler.stopSession();
    h264Stream.stopCapture();
  });

  // Remote desktop: viewer left (cleanup capture)
  socket.on('remote:leave', (data: { sessionId: string; reason?: string }) => {
    console.log('Remote session leave:', data.sessionId, data.reason);
    if (data.sessionId === currentCaptureSessionId) {
      stopCaptureSession();
      remoteHandler.stopSession();
    }
    h264Stream.stopCapture();
  ...
  // Remote desktop: H.264 hardware-accelerated streaming
  socket.on('remote:h264:start', (data: { sessionId: string; frameRate?: number; bitrate?: string }) => {
    console.log('[H264] Start request:', data.sessionId);
    h264Stream.startCapture(socket, data.sessionId, {
      frameRate: data.frameRate || 30,
      bitrate: data.bitrate || '5M',
    }).catch((err: any) => console.error('[H264] Failed to start:', err));
  });

  socket.on('remote:h264:stop', () => {
    console.log('[H264] Stop requested');
    h264Stream.stopCapture();
  });

  // Remote desktop: input events from web client (relayed through server)
  socket.on('remote:input', (data: { sessionId: string; type: string; payload: any }) => {
    remoteHandler.handleInput(data.type, data.payload);
  });

  // Remote desktop: JPEG frame request (fallback alongside WebRTC)
  socket.on('remote:frame:request', (data: { sessionId: string; quality?: number; interval?: number }) => {
    console.log('Remote frame request for session:', data.sessionId);
    // Run JPEG capture alongside WebRTC as visual fallback
    remoteHandler.stopSession();
    if (socket) {
      remoteHandler.startSession(data.sessionId, socket, {
  ...
  // WebRTC signaling: forward from API relay (from viewer) to capture renderer
  socket.on('webrtc:description', (data: { sessionId: string; from?: string; description: any }) => {
    if (data.from === 'viewer' && captureWindow && !captureWindow.isDestroyed()) {
      captureWindow.webContents.send('capture:webrtc-signal', {
        type: 'sdp',
        sessionId: data.sessionId,
        description: data.description,
      });
  ...
  socket.on('webrtc:ice-candidate', (data: { sessionId: string; from?: string; candidate: any }) => {
    if (data.from === 'viewer' && captureWindow && !captureWindow.isDestroyed()) {
      captureWindow.webContents.send('capture:webrtc-signal', {
        type: 'ice',
        sessionId: data.sessionId,
...
app.whenReady().then(() => {
  ...
  // Configure automatic screen source selection for WebRTC getDisplayMedia
  session.defaultSession.setDisplayMediaRequestHandler((_request, callback) => {
    desktopCapturer.getSources({ types: ['screen'], thumbnailSize: { width: 1, height: 1 } }).then((sources) => {
      const primary = screen.getPrimaryDisplay();
      const source = sources.find((c) => c.display_id === String(primary.id)) || sources[0] || null;
      callback({ video: source });
    }).catch((err) => {
      console.error('[Capture] displayMediaRequestHandler error:', err);
      callback(null);
    });
  });
