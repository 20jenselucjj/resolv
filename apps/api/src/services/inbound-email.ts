...
import { parseEmailCommands, validateCommandPermission, normalizeCommandValue } from './email-command-parser';
 ...
 */
function getReferences(parsed: { references?: string[] | string; messageId?: string; headers?: { name: string; value: string }[]; inReplyTo?: string }): string[] {
  const refs: string[] = [];
...
async function findTicketByReferences(parsed: { references?: string[] | string; messageId?: string }): Promise<number | null> {
 ...
 */
function extractAttachments(parsed: { parts?: { filename?: string; mimeType?: string; body?: { attachmentId?: string; size?: number }; parts?: any[] }[]; attachments?: any[] }): AttachmentInfo[] {
...
async function addCommentFromEmail(ticketNumber: number, fromEmail: string, fromName: string, body: string, messageId: string, attachments?: AttachmentInfo[]): Promise<void> {
  ...
  try {
    ...
    const parsed = parseEmailCommands('', cleanBody);
    if (parsed.commands.length > 0) {
      await executeEmailCommands(parsed.commands, ticket, user, fromEmail, client);
    }
...
async function executeEmailCommands(
  commands: { command: string; value: string; raw: string }[],
  ticket: any,
  ...
  // Get sender permissions for validation
  let senderPermissions: string[] = [];
  if (user) {
    try {
      const permResult = await client.query("SELECT value FROM system_settings WHERE key = 'role_permissions'");
      if (permResult.rows.length > 0) {
        const stored = JSON.parse(permResult.rows[0].value);
        const roleConfig = stored.find((r: any) => r.id === senderRole);
        if (roleConfig) {
          senderPermissions = roleConfig.permissions.filter((p: any) => p.enabled).map((p: any) => p.key);
        }
      }
    } catch { /* use empty permissions */ }
  }

  for (const cmd of commands) {
    try {
      // Check permission
      const permCheck = validateCommandPermission(cmd.command, senderRole, senderPermissions);
      if (!permCheck.allowed) {
        await client.query(
          `INSERT INTO email_command_log (message_id, sender_email, sender_user_id, ticket_id, command, arguments, result, error_message) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
          [null, fromEmail, user?.id || null, ticket.id, cmd.command, cmd.value, 'denied', permCheck.reason]
        );
        console.log(`[inbound-email] Email command denied: ${cmd.command} from ${fromEmail} — ${permCheck.reason}`);
        continue;
      }

      // Normalize value
      const normalized = normalizeCommandValue(cmd.command, cmd.value);
      if (!normalized.valid) {
        await client.query(
          `INSERT INTO email_command_log (sender_email, sender_user_id, ticket_id, command, arguments, result, error_message) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [fromEmail, user?.id || null, ticket.id, cmd.command, cmd.value, 'error', normalized.error]
        );
        continue;
      }
      ...
      switch (cmd.command) {
        case 'status':
          await client.query("UPDATE tickets SET status = $1, updated_at = NOW() WHERE id = $2", [normalized.normalized, ticket.id]);
          if (normalized.normalized === 'resolved') await client.query("UPDATE tickets SET resolved_at = NOW() WHERE id = $1", [ticket.id]);
          if (normalized.normalized === 'closed') await client.query("UPDATE tickets SET closed_at = NOW() WHERE id = $1", [ticket.id]);
          break;
        case 'priority':
          await client.query("UPDATE tickets SET priority = $1, updated_at = NOW() WHERE id = $2", [normalized.normalized, ticket.id]);
          break;
        case 'type':
          await client.query("UPDATE tickets SET ticket_type = $1, updated_at = NOW() WHERE id = $2", [normalized.normalized, ticket.id]);
          break;
        case 'assign': {
          const assignUser = await findUserByEmail(normalized.normalized);
          if (assignUser) {
            await client.query("UPDATE tickets SET assigned_to_id = $1, updated_at = NOW() WHERE id = $2", [assignUser.id, ticket.id]);
          }
          break;
        }
        case 'close':
          await client.query("UPDATE tickets SET status = 'closed', closed_at = NOW(), close_notes = $1, updated_at = NOW() WHERE id = $2", [cmd.value || 'Closed via email', ticket.id]);
          break;
        case 'resolve':
          await client.query("UPDATE tickets SET status = 'resolved', resolved_at = NOW(), close_notes = $1, updated_at = NOW() WHERE id = $2", [cmd.value || 'Resolved via email', ticket.id]);
          break;
        case 'reopen':
          await client.query("UPDATE tickets SET status = 'open', closed_at = NULL, resolved_at = NULL, updated_at = NOW() WHERE id = $1", [ticket.id]);
          break;
        case 'delete': {
          // Check if confirmation is required
          let confirmationRequired = true;
          try {
            const settingResult = await client.query("SELECT value FROM system_settings WHERE key = 'email_delete_confirmation_required'");
            if (settingResult.rows.length > 0) confirmationRequired = settingResult.rows[0].value === 'true';
          } catch {}
          ...
          if (confirmationRequired) {
            ...
            await client.query(
              `INSERT INTO email_delete_confirmations (ticket_id, sender_email, sender_user_id, confirmation_token, expires_at) VALUES ($1, $2, $3, $4, NOW() + INTERVAL '5 minutes')`,
              [ticket.id, fromEmail, user?.id || null, token]
            );
            await client.query(
              `INSERT INTO email_command_log (sender_email, sender_user_id, ticket_id, command, arguments, result) VALUES ($1, $2, $3, $4, $5, $6)`,
              [fromEmail, user?.id || null, ticket.id, 'delete', cmd.value, 'queued_confirmation']
            );
            console.log(`[inbound-email] Delete queued for confirmation: ticket #${ticket.number}, token sent to ${fromEmail}`);
        ...
        case 'cc': {
          ...
          const existingWatcher = await client.query(
            ...
            [ticket.id, normalized.normalized]
          );
          if (existingWatcher.rows.length === 0) {
            await client.query(
              `INSERT INTO ticket_watchers (ticket_id, email, added_by) VALUES ($1, $2, $3)`,
              [ticket.id, normalized.normalized, user?.id || null]
            );
          }
          break;
        }
        case 'category': {
          const catResult = await client.query('SELECT id FROM categories WHERE LOWER(name) = LOWER($1)', [normalized.normalized]);
          if (catResult.rows.length > 0) {
            await client.query("UPDATE tickets SET category_id = $1, updated_at = NOW() WHERE id = $2", [catResult.rows[0].id, ticket.id]);
          }
          break;
        }
      }
      ...
      // Log activity
      await client.query(
        `INSERT INTO ticket_activity (ticket_id, actor_id, action, old_value, new_value) VALUES ($1, $2, $3, $4, $5)`,
        [ticket.id, user?.id || null, `email_command_${cmd.command}`, cmd.value, normalized.normalized]
      );

      // Log success
      await client.query(
        `INSERT INTO email_command_log (sender_email, sender_user_id, ticket_id, command, arguments, result) VALUES ($1, $2, $3, $4, $5, $6)`,
        [fromEmail, user?.id || null, ticket.id, cmd.command, cmd.value, 'success']
      );

      console.log(`[inbound-email] Email command executed: ${cmd.command} ${cmd.value} on ticket #${ticket.number} by ${fromEmail}`);
    } catch (err: any) {
      await client.query(
        `INSERT INTO email_command_log (sender_email, sender_user_id, ticket_id, command, arguments, result, error_message) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
        [fromEmail, user?.id || null, ticket.id, cmd.command, cmd.value, 'error', err.message]
      ).catch(() => {});
      console.error(`[inbound-email] Email command error: ${cmd.command} — ${err.message}`);
    }
  }
}
    ...
    if (ticketNumber) {
      ...
      if (refTicket) {
        ...
        if (recentTicket) {
          ...
          // Parse subject for commands (e.g., [Priority:high] New server issue)
          const subjectCommands = parseEmailCommands(subject, '');
          const cleanSubject = subjectCommands.cleanedSubject || subject;
          ...
          if (creationEnabled.rows.length > 0 && creationEnabled.rows[0].value === 'true') {
            await createTicketFromEmail(fromEmail, fromName, cleanSubject, textBody, gmailId, emailAttachments.length > 0 ? emailAttachments : undefined);
          } else {
  ...
  if (ticketNumber) {
    ...
    if (refTicket) {
      ...
      if (recentTicket) {
        ...
        if (creationEnabled.rows.length > 0 && creationEnabled.rows[0].value === 'true') {
          await createTicketFromEmail(fromEmail, fromName, cleanSubject, textBody, messageIdHeader);
        } else {
          console.log(`[inbound-email] Skipping new email — ticket creation disabled. Subject: ${subject}`);
          const oauthEmail = await getOAuthEmail();
