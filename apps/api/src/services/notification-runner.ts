...
import { loadTemplates, findTemplate, interpolateSubject, interpolateBody, getDefaultTemplates, type TemplateVariables } from './email-template-engine';
import { fireWorkflows } from './workflow-engine';
...
export interface ApprovalNotificationEvent {
  ...
  recipientOverride?: { id?: string; email: string; name: string }[];
}
...
export async function dispatchNotifications(event: NotificationEvent): Promise<void> {
  try {
    ...
    // 10. Log all notifications for throttling
    ...
    // 11. Fire visual workflow rules (fire-and-forget)
    fireWorkflows(event).catch((err: Error) =>
      console.error('[notification-runner] Workflow dispatch failed:', err.message)
    );
...
/**
 * Dispatch approval-related notifications.
 * Call this from approval route handlers as fire-and-forget.
 */
export async function dispatchApprovalNotifications(event: ApprovalNotificationEvent): Promise<void> {
  try {
    ...
    if (event.recipientOverride && event.recipientOverride.length > 0) {
      recipients.push(...event.recipientOverride);
    } else if (event.type === 'approval_requested' && event.step?.approver_id) {
      const r = await pool.query('SELECT email, name FROM users WHERE id = $1', [event.step.approver_id]);
      if (r.rows.length > 0) recipients.push({ email: r.rows[0].email, name: r.rows[0].name });
    }

    // Also notify the requester on decision
    if (event.type === 'approval_decided') {
      const r = await pool.query('SELECT email, name FROM users WHERE id = $1', [event.approval.requested_by_id]);
      if (r.rows.length > 0) recipients.push({ email: r.rows[0].email, name: r.rows[0].name });
    }

    if (recipients.length === 0) return;

    const templateName = event.type === 'approval_requested' ? 'Approval Requested' : 'Approval Decision';

    for (const recipient of recipients) {
      const vars: Record<string, string> = {
        user_name: recipient.name,
        approval_title: event.approval.title,
        approval_description: event.approval.description || '',
        approval_url: `${webUrl}/dashboard/approvals?id=${event.approval.id}`,
        entity_type: event.approval.entity_type,
        approval_status: event.approval.status,
        approval_priority: event.approval.priority,
      };
    ...
    console.error('[notification-runner] dispatchApprovalNotifications failed:', err.message);
  }
}

// ─── Build email variables ──────────────────────────────────────────────────

async function buildEmailVariables(event: NotificationEvent): Promise<TemplateVariables> {
  const webUrl = process.env.WEB_URL || 'http://localhost:3000';
...
async function sendStandardEventEmail(
  ...
  emailVars: TemplateVariables,
  recipients: Recipient[]
): Promise<void> {
  const templateName = getTemplateNameForEvent(event.type);

  for (const recipient of recipients) {
    const vars: TemplateVariables = { ...emailVars, user_name: recipient.name };
