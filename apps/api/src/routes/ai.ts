...
export async function aiRoutes(app: FastifyInstance) {
  ...
  app.put('/ai/config', { preHandler: [app.authenticate, app.requirePermission('manage_ai_config')] }, async (req, reply) => {
...
---`
    ...
    const TOOL_HANDLERS: Record<string, ToolHandler> = {
      search_tickets: async (args) => {
        ...
        const params: any[] = []
        if (user.role === 'user') { params.push(user.id); q += ` AND (created_by_id=$${params.length} OR assigned_to_id=$${params.length})` }
      ...
      get_ticket: async (args) => {
        ...
        if (user.role === 'user' && rows[0]?.created_by_id !== user.id && rows[0]?.assigned_to_id !== user.id) {
      ...
      add_comment: async (args) => {
        ...
        const ticketQuery = isUuid
          ? 'SELECT id, number, title, created_by_id, assigned_to_id FROM tickets WHERE id=$1'
          : 'SELECT id, number, title, created_by_id, assigned_to_id FROM tickets WHERE number::text=$1'
        const { rows: tickets } = await pool.query(ticketQuery, [rawTicketId])
        if (tickets.length === 0) return { error: 'Ticket not found' }
        const ticket = tickets[0]
        if (user.role === 'user' && ticket.created_by_id !== user.id && ticket.assigned_to_id !== user.id) return { error: 'Access denied' }
    ...
    if (assistantMsg.tool_calls?.length) {
      ...
      const messages2 = [
        ...buildMessagesForAPI(sanitizedHistory, messages[0] as any),
