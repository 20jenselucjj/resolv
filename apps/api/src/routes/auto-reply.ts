...
export default async function autoReplyRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/admin/auto-replies', { preHandler: [fastify.requirePermission('manage_automation')] }, async (request, reply) => {
  ...
  fastify.patch('/admin/auto-replies/:id', { preHandler: [fastify.requirePermission('manage_automation')] }, async (request, reply) => {
    ...
    try {
      await pool.query(
        ...
        [request.user.id, 'update_auto_reply', 'auto_reply_rules', id, JSON.stringify(body)]
      );
    } catch (logErr: any) {
  ...
  fastify.delete('/admin/auto-replies/:id', { preHandler: [fastify.requirePermission('manage_automation')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    ...
    try {
      await pool.query(
        ...
        [request.user.id, 'delete_auto_reply', 'auto_reply_rules', id]
      );
    } catch (logErr: any) {
      fastify.log.error({ err: logErr }, 'Failed to write audit log');
    }
  ...
  fastify.patch('/admin/auto-replies/:id/toggle', { preHandler: [fastify.requirePermission('manage_automation')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    ...
    try {
      await pool.query(
        'INSERT INTO audit_log (actor_id, action, entity_type, entity_id, new_data) VALUES ($1, $2, $3, $4, $5)',
        [request.user.id, 'toggle_auto_reply', 'auto_reply_rules', id, JSON.stringify({ enabled: result.rows[0].enabled })]
      );
    } catch (logErr: any) {
      fastify.log.error({ err: logErr }, 'Failed to write audit log');
    }
...
// ─── DEPRECATED: Trigger auto-replies for a ticket ─────────────────────────
// Called after ticket creation. Checks all enabled rules and sends matching replies.
//
// @deprecated This function is not wired into any event handler and should not be used.
// Auto-reply rules are now handled by fireMatchingRules() in notification-runner.ts,
// which is called automatically by dispatchNotifications() on every ticket event.
// This function is retained only as a reference and may be removed in a future release.
