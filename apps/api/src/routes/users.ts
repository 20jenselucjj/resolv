...
const userUpdateSchema = z.object({
  ...
  role: z.enum(['admin', 'manager', 'agent', 'user', 'readonly']).optional(),
  department: z.string().optional(),
  phone: z.string().optional(),
  is_active: z.boolean().optional(),
  avatar_url: z.string().url().nullable().optional(),
  notification_popups: z.boolean().optional(),
});
...
export default async function userRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/users', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    let dataQuery = `
      SELECT id, email, name, role, avatar_url, department, phone, is_active, notification_popups, last_login_at, created_at, source, locked, locked_at, locked_reason 
      FROM users 
      WHERE 1=1
    `;
    const params: any[] = [];
  ...
  fastify.get('/users/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    const userResult = await pool.query(
      'SELECT id, email, name, role, avatar_url, department, phone, is_active, notification_popups, last_login_at, created_at, source, locked, locked_at, locked_reason FROM users WHERE id = $1',
      [id]
    );
  ...
  fastify.patch('/users/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    if (!isAdmin) {
      ...
      const allowedFields = ['name', 'department', 'phone', 'avatar_url', 'notification_popups'];
    ...
    try {
      const result = await pool.query(
        `UPDATE users SET ${setClauses.join(', ')} WHERE id = $1 RETURNING id, email, name, role, avatar_url, department, phone, is_active, notification_popups, created_at, source, locked`,
        [id, ...values]
      );
  ...
  fastify.post('/users/:id/reset-password', { preHandler: [fastify.requirePermission('manage_users')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  fastify.post('/users/:id/lock', { preHandler: [fastify.requirePermission('manage_users')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const currentUser = request.user as JwtPayload;
  ...
  fastify.post('/users/:id/unlock', { preHandler: [fastify.requirePermission('manage_users')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const currentUser = request.user as JwtPayload;
  ...
  fastify.delete('/users/:id', { preHandler: [fastify.requirePermission('manage_users')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    const currentUser = request.user as JwtPayload;
  ...
  fastify.post('/users/invite', { preHandler: [fastify.requirePermission('manage_users')] }, async (request, reply) => {
