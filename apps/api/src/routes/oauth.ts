...
const oauthStates = new Map<string, { codeVerifier: string; createdAt: number; mode: 'sync' | 'login' | 'email_send'; provider: 'google' | 'microsoft'; accountId?: string }>();
...
export default async function oauthRoutes(fastify: FastifyInstance) {
    ...
    try {
      ...
      const params = new URLSearchParams({
        ...
        scope: [
          ...
          'https://www.googleapis.com/auth/gmail.labels',           // NEW: manage labels
          'https://www.googleapis.com/auth/gmail.settings.basic',   // NEW: manage filters
          'https://www.googleapis.com/auth/pubsub',                 // NEW: push notifications
        ].join(' '),
        state,
  ...
  fastify.get('/oauth/google/callback', async (request, reply) => {
    try {
      ...
      // Store token metadata for health monitoring
      const tokenMetadata = {
        ...
        scopes: [
          ...
          'gmail.readonly', 'gmail.modify', 'gmail.labels', 'gmail.settings.basic', 'pubsub',
        ],
      };
      await pool.query(
        `INSERT INTO system_settings (key, value, updated_at)
         VALUES ('oauth_token_metadata', $1, NOW())
         ON CONFLICT (key) DO UPDATE SET value = $1, updated_at = NOW()`,
...
if (window.opener) {
  ...
  fastify.post('/oauth/outbound/disconnect', { preHandler: [fastify.requirePermission('manage_settings')] }, async (request, reply) => {
    try {
  ...
  fastify.get('/admin/email/oauth-status', { preHandler: [fastify.requirePermission('manage_settings')] }, async (request, reply) => {
    try {
  ...
  // GET /admin/email/token-health — OAuth token health monitoring
  fastify.get('/admin/email/token-health', { preHandler: [fastify.requirePermission('manage_settings')] }, async (request, reply) => {
    try {
      // Check all token stores
      const tokenKeys = ['directory_sync_tokens', 'smtp_oauth_tokens'];
      const health: any[] = [];

      for (const key of tokenKeys) {
        const result = await pool.query("SELECT value FROM system_settings WHERE key = $1", [key]);
        if (result.rows.length === 0) {
          health.push({ source: key, connected: false, status: 'not_configured' });
          continue;
        }
        ...
        try {
          ...
          const isExpiringSoon = !isExpired && (expiryTime - now) < 5 * 60 * 1000; // 5 min buffer
          ...
          health.push({
            ...
            expiresIn: Math.max(0, Math.round((expiryTime - now) / 1000)),
          });
        } catch {
          health.push({ source: key, connected: false, status: 'parse_error' });
        }
      }

      // Get metadata
      const metaResult = await pool.query("SELECT value FROM system_settings WHERE key = 'oauth_token_metadata'");
      let metadata = null;
      if (metaResult.rows.length > 0) {
        try { metadata = JSON.parse(metaResult.rows[0].value); } catch {}
      }
      ...
      return reply.send({
        data: {
          tokens: health,
          metadata,
          overallStatus: health.every(h => h.status === 'healthy' || h.status === 'not_configured') ? 'healthy' : 'degraded',
        },
      });
    } catch (err: any) {
  ...
  fastify.post('/oauth/google/disconnect', { preHandler: [fastify.requirePermission('manage_settings')] }, async (request, reply) => {
    try {
      await clearTokens();
