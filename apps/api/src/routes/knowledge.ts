...
export default async function knowledgeRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/knowledge', { preHandler: [fastify.requirePermission('manage_knowledge')] }, async (request, reply) => {
  ...
  fastify.patch('/knowledge/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    // Broadcast to connected clients
    fastify.io.emit('knowledge:updated', { article: updated });

    return reply.send({ data: updated });
  });

  // DELETE /knowledge/:id - admin only
  fastify.delete('/knowledge/:id', { preHandler: [fastify.requirePermission('delete_knowledge')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
    ...
    fastify.io.emit('knowledge:deleted', { id });
  ...
  fastify.post('/knowledge/images', { preHandler: [fastify.requirePermission('manage_knowledge')] }, async (request, reply) => {
  ...
  fastify.post('/knowledge/:id/attachments', { preHandler: [fastify.requirePermission('manage_knowledge')] }, async (request, reply) => {
    try {
      const { id } = request.params as { id: string };
  ...
  fastify.post('/knowledge/:id/sync-ai', { preHandler: [fastify.requirePermission('delete_knowledge')] }, async (request, reply) => {
    const { id } = request.params as { id: string };
  ...
  fastify.get('/knowledge/stats', { preHandler: [fastify.authenticate, fastify.requirePermission('view_knowledge')] }, async (request, reply) => {
