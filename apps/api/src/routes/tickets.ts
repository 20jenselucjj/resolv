...
export default async function ticketRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/tickets/bulk-delete', { preHandler: [fastify.requireRole(['admin'])] }, async (request, reply) => {
    const body = z.object({
      ids: z.array(z.string().uuid()).min(1).max(100),
    }).parse(request.body);
  ...
  fastify.delete('/tickets', { preHandler: [fastify.requireRole(['admin'])] }, async (request, reply) => {
