...
import { assignSlaPolicy } from '../services/sla-engine';
...
export default async function ticketRoutes(fastify: FastifyInstance) {
  ...
  fastify.post('/tickets', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      // Assign SLA policy based on ticket priority (if a matching active policy exists)
      // This sets sla_policy_id and recalculates due_date using business-hours calendar math
      if (ticketPriority) {
        try {
          const slaAssignment = await assignSlaPolicy(
            client,
            rawTicket.id,
            ticketPriority,
            new Date(rawTicket.created_at)
          );
          if (slaAssignment && !body.due_date) {
            // Only override due_date if the user didn't explicitly provide one
            rawTicket.due_date = slaAssignment.due_date;
            rawTicket.sla_policy_id = slaAssignment.sla_policy_id;
          }
        } catch (slaErr: any) {
          fastify.log.error({ err: slaErr, ticketId: rawTicket.id }, 'Failed to assign SLA policy');
          // Non-fatal — continue without SLA
        }
      }
  ...
  fastify.patch('/tickets/:id', { preHandler: [fastify.authenticate] }, async (request, reply) => {
    ...
    try {
      ...
      if (body.priority !== undefined && body.priority !== ticket.priority) {
        try {
          await assignSlaPolicy(
            client,
            id,
            body.priority,
            new Date(rawUpdated.created_at)
          );
        } catch (slaErr: any) {
          fastify.log.error({ err: slaErr, ticketId: id }, 'Failed to recalculate SLA on priority change');
        }
      }
