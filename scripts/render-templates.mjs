// Render all email templates with sample data and write to rendered-templates/
import { loadTemplates, findTemplate, interpolateSubject, interpolateBody } from '../apps/api/src/services/email-template-engine.js';
import fs from 'fs';
...
async function main() {
  ...
  for (const name of names) {
    ...
    fs.writeFileSync(`rendered-templates/${safeName}.html`, fullHtml, 'utf-8');
    console.log(`OK  Rendered:  rendered-templates/${safeName}.html`);
  }
  ...
  for (const name of names) {
    ...
    fs.writeFileSync(`rendered-templates/${safeName}_source.html`, sourceContent, 'utf-8');
    console.log(`OK  Source:    rendered-templates/${safeName}_source.html  (copy this into admin editor)`);
  }
}
